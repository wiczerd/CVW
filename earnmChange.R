...
sipp <- sipp[, c("panel","year","month","id","earnm","date","lfstat","wpfinwgt", "recIndic","recIndic2"), with=FALSE]

# only keep those there for a full year (take away all recession years entirely?)
sipp[, monthsInYr := .N, by=list(id,year)]
sipp[year!=shift(year), yrNo := seq(1,.N),by=list(id,year)]
sipp[, monthsInLF := sum(lfstat==1 & earnm>0), by=list(id,year)]

sipp <- sipp[monthsInYr == 12 & monthsInLF>=1,]
sipp <- sipp[ lfstat!=1, earnm:= 0]
...
sipp[, learnmQ:= shift(earnmQ), by=id]
...
sipp[, earnmDel :=-(last.earnm/earnm - 1)]

# get average earnm by earnm quantile, remove 0 earnings
df <- sipp[, .(earnmChg = wtd.quantile(earnmChg, weights=wpfinwgt, probs=c(.1,.5,.9), na.rm=T)), by=learnmQ]
df$qtl <- rep(c(10L,50L,90L),101)

# plot
ggplot(df[learnmQ>=.05 & learnmQ<=1.0,], aes(learnmQ, earnmChg,color=factor(qtl),group=factor(qtl))) + 
	geom_point() + 
	geom_line() +
	ylim(c(-1,2)) +
	...
	scale_color_manual(values = c("mediumpurple","purple","mediumpurple"),
					   ...
					   name = "")
ggsave(paste0(figuredir, "/earnmPctile_trunc2.png"),height=5,width=10)
ggsave(paste0(figuredir, "/earnmPctile_trunc2.eps"),height=5,width=10)

df <- sipp[, .(earnmDel = wtd.quantile(earnmDel, weights=wpfinwgt, probs=c(.1,.5,.9), na.rm=T)), by=earnmQ]
df$qtl <- rep(c(10L,50L,90L),101)
# plot
ggplot(df[earnmQ>=.01 & earnmQ<=1.0,], aes(earnmQ, earnmDel,color=factor(qtl),group=factor(qtl))) + 
	geom_point() + 
	geom_line() +
	ylim(c(-1,2)) +
	geom_hline(yintercept=0) +
	ggtitle("Percentiles of Annual Earnings Change by Base-Percentile") +
	xlab("Earnm percentile in base year") +
	ylab("Average percent difference from last year")+
	scale_color_manual(values = c("mediumpurple","purple","mediumpurple"),
					   labels = c("10","Median","90"),
					   name = "")
ggsave(paste0(figuredir, "/earnmPctile_Del_trunc2.png"))

# get average earnm by earnm quantile, remove 0 earnings
df2 <- sipp[, .(earnmStd = (wtd.var(earnmChg, weights=wpfinwgt, na.rm=T))^.5 ), by=learnmQ]
# plot
ggplot(df2[learnmQ>=.05 & learnmQ<=1.0,], aes(learnmQ, earnmStd)) + 
	geom_point() + 
	geom_line() +
	ylim(c(0,1.50)) +
	geom_hline(yintercept=0) +
...
dfr <- sipp[, .(earnmChg = wtd.quantile(earnmChg, weights=wpfinwgt, probs=c(.1,.5,.9), na.rm=T)), by=list(learnmQ,recIndic2)]
dfr$qtl <- rep(c(10L,50L,90L),202)

# plot
ggplot(dfr[learnmQ>=.05 & learnmQ<=1.0,], aes(learnmQ, earnmChg,color=interaction(qtl,recIndic2),group=interaction(qtl,recIndic2))) + 
	geom_point() + 
	geom_line() +
	ylim(c(-1,2)) +
	geom_hline(yintercept=0) +
	...
	scale_color_manual(values = c("steelblue","blue", "steelblue","palevioletred","red","palevioletred"),
					   ...
					   name = "")
ggsave(paste0(figuredir, "/earnmPctile_rec_trunc2.png"),height=5,width=10)
ggsave(paste0(figuredir, "/earnmPctile_rec_trunc2.eps"),height=5,width=10)
