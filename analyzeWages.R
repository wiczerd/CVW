...
incomePercentiles <- incomePercentiles %>%
        ...
        summarize(prSwitching_rec = weighted.mean(switchedOcc & (UE | EE) & Rec , wpfinwgt, na.rm = TRUE)) %>%
        summarize(prSwitching_exp = weighted.mean(switchedOcc & (UE | EE) & !Rec, wpfinwgt, na.rm = TRUE))
