...
PsRec <- rep(0,NS)
for (swi in c(0,1)){
	PsExp[1+3*swi] = wcExp[!is.na(wagechange), wtd.mean(switchedOcc==as.logical(1-swi) & EE==T & UE==F & EU ==F,balanceweight)]
	PsExp[2+3*swi] = wcExp[!is.na(wagechange), wtd.mean(switchedOcc==as.logical(1-swi) & EE==F & UE==T & EU ==F,balanceweight)]
	PsExp[3+3*swi] = wcExp[!is.na(wagechange), wtd.mean(switchedOcc==as.logical(1-swi) & EE==F & UE==F & EU ==T,balanceweight)]
	PsRec[1+3*swi] = wcRec[!is.na(wagechange), wtd.mean(switchedOcc==as.logical(1-swi) & EE==T & UE==F & EU ==F,balanceweight)]
	PsRec[2+3*swi] = wcRec[!is.na(wagechange), wtd.mean(switchedOcc==as.logical(1-swi) & EE==F & UE==T & EU ==F,balanceweight)]
	PsRec[3+3*swi] = wcRec[!is.na(wagechange), wtd.mean(switchedOcc==as.logical(1-swi) & EE==F & UE==F & EU ==T,balanceweight)]
}
...
for (q in qtls){
	...
	share[qi] = 0.
	for(si in seq(1,NS)){
		share[qi] = PsExp[si]*(distExpS[[si]](wcE)-distExpS[[si]](wcE) ) + PsRec[si]*(distExpS[[si]](wcR)-distExpS[[si]](wcR)) + share[qi]
	}
