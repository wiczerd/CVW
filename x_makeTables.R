...
toKeep_wave <- c("switchedOcc_wave","switched_wave","switched_anan","esr_max",
            ...
            "unrt","wpfinwgt","perwt","truncweight","cleaningtruncweight","lastann.wavewage","matched_EUUE_anan","lastann.rawwg",
...
DTseam[ changer_anan & (EU_wave==F & nextann.wavewage<= 0) | lastann.wavewage<minLEarn ,changer_anan:= NA]
DTseam[ stayer_anan  & (EU_wave==F & nextann.wavewage<= 0) | lastann.wavewage<minLEarn ,stayer_anan:= NA]
...
for( wc in c("wagechange_anan","wagechangeEUE_wave","rawwgchangeEUE_wave","rawwgchange_anan") ){
	...
	######################################################################
	#Wage moments Overall, StCh , Occ Chng, Rec/Exp-----------------------
	######################################################################
	#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	if(bootse == T){
		...
		se_occswmoments <- array(0.,dim = c(18,5,Nsim))
	}
	tab_wavemoments  <- array(0., dim=c(9,5))
	tab_occswmoments <- array(0., dim=c(18,5))
	for( si in seq(1,bootse*Nsim+1) ){
		if(si>1){
			seedint = seedint+1
			set.seed(seedint)
		...
		tab_wavemoments[1,1]    <- DThr[(st|ch)&demo==T,       wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)) )]
		tab_wavemoments[1,2:5]  <- DThr[(st|ch)&demo==T,wtd.4qtlmoments(eval(as.name(wc)),eval(as.name(wt)))]
		tab_wavemoments[2,1]    <- DThr[ st ==T&demo==T,       wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
		...
		for(rI in c(F,T)){
			rix = as.integer(rI)*3+3
			tab_wavemoments[1+rix,1]    <- DThr[eval(as.name(recDef)) == rI & (st|ch)&demo==T,        wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)) )]
			tab_wavemoments[1+rix,2:5]  <- DThr[eval(as.name(recDef)) == rI & (st|ch)&demo==T,wtd.4qtlmoments(eval(as.name(wc)),eval(as.name(wt)))]
			tab_wavemoments[2+rix,1]    <- DThr[eval(as.name(recDef)) == rI &  st ==T&demo==T,       wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
		...
		if(si>1){
			...
			se_occswmoments[,,si-1] = tab_occswmoments
		}else{
			dat_wavemoments <- tab_wavemoments
			dat_occswmoments <- tab_occswmoments
		}
	}
	...
	tab_wavemomentsci_in <- array(0.,dim=c(2*nrow(tab_wavemoments),2*ncol(tab_wavemoments)))
	...
	if(bootse == T){
		for( ri in seq(0,nrow(tab_wavemoments)-1) ){
			for(ci in seq( 0,ncol(tab_wavemoments)-1 )){
				tab_wavemomentsci   [ ri*2+1,ci  +1 ] <- tab_wavemoments[ri+1,ci+1]
				tab_wavemomentsci_in[ ri*2+1,ci*2+1 ] <- tab_wavemoments[ri+1,ci+1]
				tab_wavemomentsci_in[ ri*2+2,(ci*2+1):(ci*2+2) ] <- quantile(se_wavemoments[ri+1,ci+1, ], probs=c(0.025,0.975))
				tab_wavemomentsse[ ri*2+1,ci+1 ] <- tab_wavemoments[ri+1,ci+1]
				tab_wavemomentsse[ ri*2+2,ci+1 ] <- var(se_wavemoments[ri+1,ci+1, ])^0.5
			}
		}
		for(ri in seq(0,nrow(tab_occswmoments)-1)){
			for(ci in seq(0,ncol(tab_occswmoments)-1)){
				tab_occswmomentsci   [ ri*2+1,ci  +1 ] <- tab_occswmoments[ri+1,ci+1]
				tab_occswmomentsci_in[ ri*2+1,ci*2+1 ] <- tab_occswmoments[ri+1,ci+1]
				tab_occswmomentsci_in[ ri*2+2,(ci*2+1):(ci*2+2) ] <- quantile(se_occswmoments[ri+1,ci+1, ], probs=c(0.025,0.975))
			}
		}
	
		#output it to tables
		tab_wavemomentsse<-round(tab_wavemomentsse,digits=3)
		tab_wavemomentsci<-round(tab_wavemomentsci,digits=3)
		tab_wavemomentsci_in<-round(tab_wavemomentsci_in,digits=3)
		
		tab_occswmomentsci<-round(tab_occswmomentsci,digits=3)
		tab_occswmomentsci_in<-round(tab_occswmomentsci_in,digits=3)
		...
		tab_occswmomentsci <-parensLR(tab_occswmomentsci_in,tab_occswmomentsci)
	...
	tab_stswqtldec <- array(NA_real_,dim=c((Ndifs+1)*2,3))
	...
	condvar  <- DTseam[(st|ch)&(sw|!sw)&demo==T&(sw|!sw)&(eval(as.name(wc)) > qtlshere[Nqtls-rri+1] | eval(as.name(wc)) < qtlshere[rri]), sum(eval(as.name(wt))*(eval(as.name(wc))- totmean)^2,na.rm=T) ]
	...
	for(rri in seq(1,Ndifs-1)){
		condmean <- DTseam[(st|ch)&(sw|!sw)&demo==T&(sw|!sw)&(eval(as.name(wc)) < qtlshere[Nqtls-rri+1] & eval(as.name(wc)) > qtlshere[rri]), wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt))) ]
		condvar  <- DTseam[(st|ch)&(sw|!sw)&demo==T&(sw|!sw)&(eval(as.name(wc)) < qtlshere[Nqtls-rri+1] & eval(as.name(wc)) > qtlshere[rri]), sum(eval(as.name(wt))*(eval(as.name(wc))- totmean)^2,na.rm=T) ]
		condwt   <- DTseam[(st|ch)&(sw|!sw)&demo==T&(sw|!sw)&(eval(as.name(wc)) < qtlshere[Nqtls-rri+1] & eval(as.name(wc)) > qtlshere[rri]), sum(eval(as.name(wt)),na.rm=T) ]
		
		tab_waveqtldec[rri+1,1] <- DTseam[st ==T               &(sw|!sw)&demo==T& (eval(as.name(wc)) < qtlshere[Nqtls-rri+1] & eval(as.name(wc)) > qtlshere[rri]), sum(eval(as.name(wt))*(eval(as.name(wc)) - condmean)^2,na.rm=T)]/condvar
		tab_waveqtldec[rri+1,2] <- DTseam[EEfrq==T & ch==T     &(sw|!sw)&demo==T& (eval(as.name(wc)) < qtlshere[Nqtls-rri+1] & eval(as.name(wc)) > qtlshere[rri]), sum(eval(as.name(wt))*(eval(as.name(wc)) - condmean)^2,na.rm=T)]/condvar
		tab_waveqtldec[rri+1,3] <- DTseam[(EUfrq|UEfrq)& ch==T &(sw|!sw)&demo==T& (eval(as.name(wc)) < qtlshere[Nqtls-rri+1] & eval(as.name(wc)) > qtlshere[rri]), sum(eval(as.name(wt))*(eval(as.name(wc)) - condmean)^2,na.rm=T)]/condvar
		for( swi in c(T,F)){
			tab_stswqtldec[(rri)*2+swi+1,1] <- DTseam[st ==T              & sw==swi &demo==T& (eval(as.name(wc)) < qtlshere[Nqtls-rri+1] & eval(as.name(wc)) > qtlshere[rri]), sum(eval(as.name(wt))*(eval(as.name(wc)) - condmean)^2,na.rm=T)]/condvar
			tab_stswqtldec[(rri)*2+swi+1,2] <- DTseam[EEfrq==T & ch==T    & sw==swi &demo==T& (eval(as.name(wc)) < qtlshere[Nqtls-rri+1] & eval(as.name(wc)) > qtlshere[rri]), sum(eval(as.name(wt))*(eval(as.name(wc)) - condmean)^2,na.rm=T)]/condvar
			tab_stswqtldec[(rri)*2+swi+1,3] <- DTseam[(EUfrq|UEfrq)& ch==T& sw==swi &demo==T& (eval(as.name(wc)) < qtlshere[Nqtls-rri+1] & eval(as.name(wc)) > qtlshere[rri]), sum(eval(as.name(wt))*(eval(as.name(wc)) - condmean)^2,na.rm=T)]/condvar
		}
	}
	...
	rownames(tab_chngvarqtldec) <- c("Variance","0.0-0.05,0.95-1.0","0.95-0.05","0.9-0.1","0.75-0.25","Pop")# "Variance\ ","0.95-0.05\ ","0.9-0.1\ ","0.75-0.25\ ","Pct Sample")
	rownames(tab_stswchngvarqtldec) <- c("Variance","","0.0-0.05,0.95-1.0","\ \ \ \ \ ","0.95-0.05","\ ","0.9-0.1","\ \ ","0.75-0.25","\ \ \ ","Pop","\ \ \ \ ")# "Variance\ ","0.95-0.05\ ","0.9-0.1\ ","0.75-0.25\ ","Pct Sample")
	
	
	tab_chngvarqtldec <- xtable(tab_chngvarqtldec, digits=3, 
	                            align="l|l|ll", caption="Decomposition of earnings change dispersion \\label{tab:wavechngvarqtldec}")

	tab_stswchngvarqtldec <- xtable(tab_stswchngvarqtldec, digits=3, 
	...
	if(demolbl>=1 & demolbl<=7){
		...
		print(tab_stswchngvarqtldec,include.rownames=T, hline.after= c(0,nrow(tab_stswchngvarqtldec)-2, nrow(tab_stswchngvarqtldec)), file=paste0(outputdir,"/",nametab, demotxt[demolbl],"_",wclab,".tex")) 
	}else{
		...
		nametab <- "stswchngvarqtldec"
		print(tab_stswchngvarqtldec,include.rownames=T, hline.after= c(0,nrow(tab_stswchngvarqtldec)-2, nrow(tab_stswchngvarqtldec)), file=paste0(outputdir,"/",nametab,"_",wclab,".tex") ) 
	}
	

	# Var decomp recession and expansion ----------------------------------------------
	for (rI in c(F,T)){
	  ...
	  tab_wavevardec[1,1] <- DTseam[(EEfrq==F&EUfrq==F &UEfrq==F) & recIndic_wave==rI, sum(eval(as.name(wt))*(eval(as.name(wc)) - totmean)^2,na.rm=T) ]/totvar
	  tab_wavevardec[1,2] <- DTseam[(EEfrq==T&EUfrq==F &UEfrq==F) & recIndic_wave==rI, sum(eval(as.name(wt))*(eval(as.name(wc)) - totmean)^2,na.rm=T) ]/totvar
	  tab_wavevardec[1,3] <- DTseam[(EEfrq==F&(EUfrq==T|UEfrq==T))& recIndic_wave==rI, sum(eval(as.name(wt))*(eval(as.name(wc)) - totmean)^2,na.rm=T) ]/totvar
	  totwt <- DTseam[recIndic_wave==rI, sum(eval(as.name(wt)),na.rm=T) ]
	  tab_wavevardec[2,1] <- DTseam[(EEfrq==F&EUfrq==F &UEfrq==F) & recIndic_wave==rI, sum(eval(as.name(wt)),na.rm=T) ]/totwt
	  tab_wavevardec[2,2] <- DTseam[(EEfrq==T&EUfrq==F &UEfrq==F) & recIndic_wave==rI, sum(eval(as.name(wt)),na.rm=T) ]/totwt
	  tab_wavevardec[2,3] <- DTseam[(EEfrq==F&(EUfrq==T|UEfrq==T))& recIndic_wave==rI, sum(eval(as.name(wt)),na.rm=T) ]/totwt
	  ...
	  for(rri in seq(1,Ndifs)){
	    tab_waveqtldec[rri,1] <- DTseam[recIndic_wave==rI & (EEfrq==F&EUfrq==F &UEfrq==F)  &demo==T& (eval(as.name(wc)) > qtlshere[Nqtls-rri+1] | eval(as.name(wc)) < qtlshere[rri]), sum(eval(as.name(wt)) ,na.rm=T)]
	    tab_waveqtldec[rri,2] <- DTseam[recIndic_wave==rI & (EEfrq==T&EUfrq==F &UEfrq==F)  &demo==T& (eval(as.name(wc)) > qtlshere[Nqtls-rri+1] | eval(as.name(wc)) < qtlshere[rri]), sum(eval(as.name(wt)) ,na.rm=T)]
	    tab_waveqtldec[rri,3] <- DTseam[recIndic_wave==rI & (EEfrq==F&(EUfrq==T|UEfrq==T)) &demo==T& (eval(as.name(wc)) > qtlshere[Nqtls-rri+1] | eval(as.name(wc)) < qtlshere[rri]), sum(eval(as.name(wt)) ,na.rm=T)]
	  }
	  tab_waveqtldec[Ndifs+1,1] <- DTseam[recIndic_wave==rI &  (st==T)                                        &demo==T, sum(eval(as.name(wt)),na.rm=T) ]
	  tab_waveqtldec[Ndifs+1,2] <- DTseam[recIndic_wave==rI &  (EEfrq&ch)                      &demo==T, sum(eval(as.name(wt)),na.rm=T) ]
	  tab_waveqtldec[Ndifs+1,3] <- DTseam[recIndic_wave==rI & ((EUfrq|UEfrq)&ch)&demo==T, sum(eval(as.name(wt)),na.rm=T) ]
	...
	totmean <- DTseam[(st|ch)&demo==T&(sw|!sw), wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt))) ]
	totskew <- DTseam[(st|ch)&demo==T&(sw|!sw), sum(eval(as.name(wt))*(eval(as.name(wc))- totmean)^3,na.rm=T) ]
	totwt   <- DTseam[(st|ch)&demo==T&(sw|!sw), sum(eval(as.name(wt)),na.rm=T) ]
	...
	tab_waveskewdec[1,1] <- DTseam[st == T              &(sw|!sw)&demo==T, sum(eval(as.name(wt))*(eval(as.name(wc)) - totmean)^3,na.rm=T) ]/totskew
	tab_waveskewdec[1,2] <- DTseam[ch == T & EEfrq ==T  &(sw|!sw)&demo==T, sum(eval(as.name(wt))*(eval(as.name(wc)) - totmean)^3,na.rm=T) ]/totskew
	tab_waveskewdec[1,3] <- DTseam[ch == T&(EUfrq|UEfrq)&(sw|!sw)&demo==T, sum(eval(as.name(wt))*(eval(as.name(wc)) - totmean)^3,na.rm=T) ]/totskew
	
	tab_waveskewdec[2,1] <- DTseam[st ==T               &(sw|!sw)&demo==T, sum(eval(as.name(wt)),na.rm=T) ]/totwt
	tab_waveskewdec[2,2] <- DTseam[ch ==T & EEfrq       &(sw|!sw)&demo==T, sum(eval(as.name(wt)),na.rm=T) ]/totwt
	tab_waveskewdec[2,3] <- DTseam[ch ==T &(EUfrq|UEfrq)&(sw|!sw)&demo==T, sum(eval(as.name(wt)),na.rm=T) ]/totwt
	for(swi in c(T,F)){
		tab_stswskewdec[swi+1,1] <- DTseam[st ==T               &sw==swi&demo==T, sum(eval(as.name(wt))*(eval(as.name(wc)) - totmean)^3,na.rm=T) ]/totskew
		tab_stswskewdec[swi+1,2] <- DTseam[ch ==T & EEfrq ==T   &sw==swi&demo==T, sum(eval(as.name(wt))*(eval(as.name(wc)) - totmean)^3,na.rm=T) ]/totskew
		tab_stswskewdec[swi+1,3] <- DTseam[ch ==T &(EUfrq|UEfrq)&sw==swi&demo==T, sum(eval(as.name(wt))*(eval(as.name(wc)) - totmean)^3,na.rm=T) ]/totskew
		
		tab_stswskewdec[swi+3,1] <- DTseam[st ==T               &sw==swi&demo==T, sum(eval(as.name(wt)),na.rm=T) ]/totwt
		tab_stswskewdec[swi+3,2] <- DTseam[ch ==T & EEfrq       &sw==swi&demo==T, sum(eval(as.name(wt)),na.rm=T) ]/totwt
		tab_stswskewdec[swi+3,3] <- DTseam[ch ==T &(EUfrq|UEfrq)&sw==swi&demo==T, sum(eval(as.name(wt)),na.rm=T) ]/totwt
	}
	...
	for(ri in seq(1,nrow(tab_stswchngskewqtldec),2)){
		rescale <- (1-sum(tab_stswchngskewqtldec[ri:(ri+1),1]))/sum(tab_stswchngskewqtldec[ri:(ri+1),2:3])
		tab_stswchngskewqtldec[ri:(ri+1),2:3] <- tab_stswchngskewqtldec[ri:(ri+1),2:3]*rescale
	}
	
	tab_chngskewqtldec<- data.table(tab_chngskewqtldec)
	tab_stswchngskewqtldec<- data.table(tab_stswchngskewqtldec)
	if(wc=="wagechangeEUE_wave"|wc=="rawwgchangeEUE_wave"){
		names(tab_chngskewqtldec) <- c("Job\ Stayers","EE","EUE")	
		names(tab_stswchngskewqtldec) <- c("Job\ Stayers","EE","EUE")
	}else{
		names(tab_chngskewqtldec) <- c("Job\ Stayers","EE","EU,UE")	
		names(tab_stswchngskewqtldec) <- c("Job\ Stayers","EE","EU,UE")	
	}
	...
	tab_chngskewqtldec <- xtable(tab_chngskewqtldec, digits=3, 
								align="l|l|ll", caption="Decomposition of earnings change dispersion \\label{tab:wavechngskewqtldec}")
	
	tab_stswchngskewqtldec <- xtable(tab_stswchngskewqtldec, digits=3, 
									align="l|l|ll", caption="Decomposition of earnings change dispersion, occupation stayers and switchers \\label{tab:stswchngskewqtldec}")
	
	nametab <- "chngskewqtldec"
	if(demolbl>=1 & demolbl<=7){
		print(tab_chngskewqtldec,include.rownames=T, hline.after= c(0,nrow(tab_chngskewqtldec)-1, nrow(tab_chngskewqtldec)), file=paste0(outputdir,"/",nametab, demotxt[demolbl],"_",wclab,".tex")) 
		nametab <- "stswchngskewqtldec"
		print(tab_stswchngskewqtldec,include.rownames=T, hline.after= c(0,nrow(tab_stswchngskewqtldec)-2, nrow(tab_stswchngskewqtldec)), file=paste0(outputdir,"/",nametab, demotxt[demolbl],"_",wclab,".tex")) 
	}else{
		print(tab_chngskewqtldec,include.rownames=T, hline.after= c(0,nrow(tab_chngskewqtldec)-1, nrow(tab_chngskewqtldec)), file=paste0(outputdir,"/",nametab,"_",wclab,".tex") ) 
		nametab <- "stswchngskewqtldec"
		print(tab_stswchngskewqtldec,include.rownames=T, hline.after= c(0,nrow(tab_stswchngskewqtldec)-2, nrow(tab_stswchngskewqtldec)), file=paste0(outputdir,"/",nametab,"_",wclab,".tex") ) 
	}
	...
	tab_wavestaydist    <- array(0.,dim=c(3,tN))
	...
	for(si in seq(1,bootse*Nsim+1)){
		...
		tab_wavestaydist[3,2:tN] <- DThr[st==T&demo==T & sw==T,wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs=tabqtls)]
	...
	#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	dat_wavedist <- data.table(dat_wavechngdist[seq(3,9,3),])
	names(dat_wavedist) <- c("Mean","P10","P25","P50","P75","P90")
	...
	plt_stsw_staychng_rec   <- array(NA, dim=c(8,tN))
	plt_stsw_EEEU_rec   <- array(NA, dim=c(8,tN))
	
	tab_stsw_staychng_rec   <- array(NA, dim=c(8,5)) #table 1, bottom half
	...
	if(bootse == T){
		...
		ci_stsw_staychng_rec   <- array(NA, dim=c(8,5)) #table 1, bottom half
	}
	...
	for( si in seq(1,bootse*Nsim+1) ){
		...
		for(AllEEEU in c(1,2,3)){
			if(AllEEEU==1){
				...
				DThr[ , trX_indic := NULL]
				DThr[                   , trX_indic := F]
				DThr[ EEfrq==T & demo==T, trX_indic := T]
			}else if(AllEEEU==3){
				DThr[ , trX_indic := NULL]
				DThr[                            , trX_indic := F]
				DThr[ (EUfrq==T|UEfrq==T)&demo==T, trX_indic := T]
			}
			eidx = 9*(AllEEEU-1)
	
			tab_wavechngdist_rec[1+eidx,1   ] <- DThr[ch ==T &trX_indic==T&(sw|!sw),    wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
			tab_wavechngdist_rec[1+eidx,2:tN] <- DThr[ch ==T &trX_indic==T&(sw|!sw),wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs= tabqtls)]
			tab_wavechngdist_rec[2+eidx,1   ] <- DThr[ch ==T &trX_indic==T &sw==F  ,    wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
			tab_wavechngdist_rec[2+eidx,2:tN] <- DThr[ch ==T &trX_indic==T &sw==F  ,wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs=tabqtls)]
			tab_wavechngdist_rec[3+eidx,1   ] <- DThr[ch ==T &trX_indic==T &sw==T  ,    wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
			tab_wavechngdist_rec[3+eidx,2:tN] <- DThr[ch ==T &trX_indic==T &sw==T  ,wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs=tabqtls)]
			for(rI in c(F,T)){
				ridx = as.integer(rI)*3+3
				tab_wavechngdist_rec[1+ridx+eidx,1   ] <- DThr[ch ==T &trX_indic==T&(sw|!sw)&eval(as.name(recDef))  == rI,    wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
				tab_wavechngdist_rec[1+ridx+eidx,2:tN] <- DThr[ch ==T &trX_indic==T&(sw|!sw)&eval(as.name(recDef))  == rI,wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs= tabqtls)]
				tab_wavechngdist_rec[2+ridx+eidx,1   ] <- DThr[ch ==T &trX_indic==T &sw==F  &eval(as.name(recDef))  == rI,    wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
				tab_wavechngdist_rec[2+ridx+eidx,2:tN] <- DThr[ch ==T &trX_indic==T &sw==F  &eval(as.name(recDef))  == rI,wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs=tabqtls)]
				tab_wavechngdist_rec[3+ridx+eidx,1   ] <- DThr[ch ==T &trX_indic==T &sw==T  &eval(as.name(recDef))  == rI,    wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
				tab_wavechngdist_rec[3+ridx+eidx,2:tN] <- DThr[ch ==T &trX_indic==T &sw==T  &eval(as.name(recDef))  == rI,wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs=tabqtls)]
				if(AllEEEU == 1 & si==1){
					plt_stsw_staychng_rec[3+rI,1]    <- tab_wavechngdist_rec[2+ridx+eidx,1]
					plt_stsw_staychng_rec[3+rI,2:tN] <- tab_wavechngdist_rec[2+ridx+eidx,2:tN]
					plt_stsw_staychng_rec[7+rI,1]    <- tab_wavechngdist_rec[3+ridx+eidx,1]
					plt_stsw_staychng_rec[7+rI,2:tN] <- tab_wavechngdist_rec[3+ridx+eidx,2:tN]
				}
				if(AllEEEU >1& si==1 ){
					...
					plt_stsw_EEEU_rec[1+(AllEEEU-2)*2+rI,1   ] <- tab_wavechngdist_rec[2+ridx+eidx,1   ] 
					plt_stsw_EEEU_rec[1+(AllEEEU-2)*2+rI,2:tN] <- tab_wavechngdist_rec[2+ridx+eidx,2:tN]
					plt_stsw_EEEU_rec[5+(AllEEEU-2)*2+rI,1   ] <- tab_wavechngdist_rec[3+ridx+eidx,1   ] 
					plt_stsw_EEEU_rec[5+(AllEEEU-2)*2+rI,2:tN] <- tab_wavechngdist_rec[3+ridx+eidx,2:tN]
				}
			}
			#wage moments:
			tab_wavechngmoments_rec[1+eidx,1]    <- DThr[ch==T &trX_indic==T &!is.na(sw),       wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)) )]
			tab_wavechngmoments_rec[1+eidx,2:5]  <- DThr[ch==T &trX_indic==T &!is.na(sw),wtd.4qtlmoments(eval(as.name(wc)),eval(as.name(wt)))]
			tab_wavechngmoments_rec[2+eidx,1]    <- DThr[ch==T &trX_indic==T &sw==F     ,       wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
			tab_wavechngmoments_rec[2+eidx,2:5]  <- DThr[ch==T &trX_indic==T &sw==F     ,wtd.4qtlmoments(eval(as.name(wc)),eval(as.name(wt)))]
			tab_wavechngmoments_rec[3+eidx,1]    <- DThr[ch==T &trX_indic==T &sw==T     ,       wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
			tab_wavechngmoments_rec[3+eidx,2:5]  <- DThr[ch==T &trX_indic==T &sw==T     ,wtd.4qtlmoments(eval(as.name(wc)),eval(as.name(wt)))]
		
			#expansion/recession
			for(rI in c(F,T)){
				ridx = rI*3+3
				tab_wavechngmoments_rec[1+eidx+ridx,1]    <- DThr[ch==T &trX_indic==T &!is.na(sw) &eval(as.name(recDef))  == rI,        wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)) )]
				tab_wavechngmoments_rec[1+eidx+ridx,2:5]  <- DThr[ch==T &trX_indic==T &!is.na(sw) &eval(as.name(recDef))  == rI,wtd.4qtlmoments(eval(as.name(wc)),eval(as.name(wt)))]
				tab_wavechngmoments_rec[2+eidx+ridx,1]    <- DThr[ch==T &trX_indic==T &sw==F      &eval(as.name(recDef))  == rI,       wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
				tab_wavechngmoments_rec[2+eidx+ridx,2:5]  <- DThr[ch==T &trX_indic==T &sw==F      &eval(as.name(recDef))  == rI,wtd.4qtlmoments(eval(as.name(wc)),eval(as.name(wt)))]
				tab_wavechngmoments_rec[3+eidx+ridx,1]    <- DThr[ch==T &trX_indic==T &sw==T      &eval(as.name(recDef))  == rI,       wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
				tab_wavechngmoments_rec[3+eidx+ridx,2:5]  <- DThr[ch==T &trX_indic==T &sw==T      &eval(as.name(recDef))  == rI,wtd.4qtlmoments(eval(as.name(wc)),eval(as.name(wt)))]
			}
		} #All, EE,EU
		# split stayers
		tab_wavestaydist_rec[1,1   ] <- DThr[st==T &demo==T&(sw|!sw),    wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
		tab_wavestaydist_rec[1,2:tN] <- DThr[st==T &demo==T&(sw|!sw),wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs= tabqtls)]
		tab_wavestaydist_rec[2,1   ] <- DThr[st==T &demo==T &sw==F  ,    wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
		tab_wavestaydist_rec[2,2:tN] <- DThr[st==T &demo==T &sw==F  ,wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs=tabqtls)]
		tab_wavestaydist_rec[3,1   ] <- DThr[st==T &demo==T &sw==T  ,    wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
		tab_wavestaydist_rec[3,2:tN] <- DThr[st==T &demo==T &sw==T  ,wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs=tabqtls)]
		for(rI in c(F,T)){
			ridx = as.integer(rI)*3+3
			tab_wavestaydist_rec[1+ridx,1   ] <- DThr[st==T &demo==T&(sw|!sw)&eval(as.name(recDef))  == rI,    wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
			tab_wavestaydist_rec[1+ridx,2:tN] <- DThr[st==T &demo==T&(sw|!sw)&eval(as.name(recDef))  == rI,wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs= tabqtls)]
			tab_wavestaydist_rec[2+ridx,1   ] <- DThr[st==T &demo==T &sw==F  &eval(as.name(recDef))  == rI,    wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
			tab_wavestaydist_rec[2+ridx,2:tN] <- DThr[st==T &demo==T &sw==F  &eval(as.name(recDef))  == rI,wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs=tabqtls)]
			tab_wavestaydist_rec[3+ridx,1   ] <- DThr[st==T &demo==T &sw==T  &eval(as.name(recDef))  == rI,    wtd.mean(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)))]
			tab_wavestaydist_rec[3+ridx,2:tN] <- DThr[st==T &demo==T &sw==T  &eval(as.name(recDef))  == rI,wtd.quantile(eval(as.name(wc)),na.rm=T,weights=eval(as.name(wt)), probs=tabqtls)]
			...
			plt_stsw_staychng_rec[1+rI,1]    <- tab_wavestaydist_rec[2+ridx,1]
			plt_stsw_staychng_rec[1+rI,2:tN] <- tab_wavestaydist_rec[2+ridx,2:tN]
			plt_stsw_staychng_rec[5+rI,1]    <- tab_wavestaydist_rec[3+ridx,1]
			plt_stsw_staychng_rec[5+rI,2:tN] <- tab_wavestaydist_rec[3+ridx,2:tN]
	...
	#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	#panels (d),(e)
	dat_wavedist <- data.table(plt_stsw_EEEU_rec[1:4,])
	names(dat_wavedist) <- c("Mean","P10","P25","P50","P75","P90")
	dat_wavedist[ , cat := as.factor(c("EE","EE","EU,UE","EU,UE"))]
	...
	ggplot( dat_wavedist , aes(cat)) + theme_bw()+ylim(c(min(plt_stsw_EEEU_rec[,2])*1.05,max(plt_stsw_EEEU_rec[,6])*1.05))+
		geom_boxplot(aes( ymin=P10,lower=P25,middle=Mean,upper=P75,ymax=P90 ,color=Cycle),stat="identity",fill="darkgreen",alpha=0.4)+ #
	...
	dat_wavedist[ , Cycle := as.factor(c("Expansion","Recession","Expansion","Recession"))]
	ggplot( dat_wavedist , aes(cat)) + theme_bw()+ylim(c(min(plt_stsw_EEEU_rec[,2])*1.05,max(plt_stsw_EEEU_rec[,6])*1.05))+
		geom_boxplot(aes( ymin=P10,lower=P25,middle=Mean,upper=P75,ymax=P90 ,color=Cycle),stat="identity", fill= "maroon",alpha=0.4)+ #
		scale_color_manual(values=c("blue","red"))+
		xlab("")+ylab("Log Earnings Change")#scale_x_discrete(labels=c(" ","No Switch, Rec","Switchers, Exp","Switchers, Rec"))+
	...
	names(dat_wavedist) <- c("Mean","P10","P25","P50","P75","P90")
	dat_wavedist[ , cat := as.factor(c("Employer Stayers","Employer Stayers","Employer Movers","Employer Movers"))]
	dat_wavedist[ , Cycle := as.factor(c("Expansion","Recession","Expansion","Recession"))]
	ggplot( dat_wavedist , aes(cat)) + theme_bw()+ylim(c(min(plt_stsw_EEEU_rec[,2])*1.05,max(plt_stsw_EEEU_rec[,6])*1.05))+
		geom_boxplot(aes( ymin=P10,lower=P25,middle=Mean,upper=P75,ymax=P90 ,color=Cycle),stat="identity",fill="darkgreen",alpha=0.4)+ #
		scale_color_manual(values=c("blue","red"))+
	...
	ggplot( dat_wavedist , aes(cat)) + theme_bw()+
		geom_boxplot(aes( ymin=P10,lower=P25,middle=Mean,upper=P75,ymax=P90 ,color=Cycle),stat="identity")+
		scale_color_manual(values=c("blue","red"))+
		scale_x_discrete(labels=c("No Switch, Exp","No Switch, Rec","Switchers, Exp","Switchers, Rec"))+xlab("")+ylab("Log Earnings Change")
	nametab = "box_stayrec"
	...
	for(AllEEEU in c(1,2,3)){
		...
		if(bootse == T){
			...
			#pre-round it:
			tab_wavedistse <- round(tab_wavedistse,digits=3)
			...
			rowtitles <- list( pos=list(0,6,12), command=c("\\hline  \\color{Maroon}{1996-2012} &  & & & & & \\\\ \n",
														   ...
														   "\\hline \\hline   \\color{Maroon}{Recession} &  & & & & & \\\\  \n")  )
			rownames(tab_wavedistse) <- rnames
			rownames(tab_wavedistci) <- rnames
		...
		if(bootse == T){
			for( ri in seq(0,nrow(tab_wavemoments)-1) ){
				for(ci in seq( 0,ncol(tab_wavemoments)-1 )){
					tab_wavemomentsci_in[ ri*2+1,ci*2+1 ] <- tab_wavemoments[ri+1,ci+1]
					tab_wavemomentsci_in[ ri*2+2,(ci*2+1):(ci*2+2) ] <- quantile(se_wavechngmoments[ri+1,ci+1, ], probs=c(0.05,0.95))
					tab_wavemomentsci[ ri*2+1,ci+1 ] <- tab_wavemoments[ri+1,ci+1]
					tab_wavemomentsse[ ri*2+1,ci+1 ] <- tab_wavemoments[ri+1,ci+1]
	...
	#age-specific stuff for WSJ:
	ageGrp_moments <- array(0.,dim=c(3,5))
	for(ai in seq(1,3)){
		ageGrp_moments[ai,1]   <- DTseam[ ageGrp==ai & EUfrq==T & ch==T , wtd.mean(eval(as.name(wc)),weights=eval(as.name(wt)),na.rm=T)]
		ageGrp_moments[ai,2:5] <- DTseam[ ageGrp==ai & EUfrq==T & ch==T , wtd.4qtlmoments(eval(as.name(wc)),eval(as.name(wt)))]
	}
	ageGrp_moments <- data.table(ageGrp_moments)
	names(ageGrp_moments) <- c("Mean","Median","Median Abs Deviaion","Goeneveld Meeden","Moors")
	rownames(ageGrp_moments) <- c("30 and under","31-55","56+")
	tab_wavemoments <- xtable(ageGrp_moments, digits=2, 
								align="l|l|llll", caption="Distribution of quarter-quarter log earnings changes, absent unemployment")
	nametab <- "ageGrp_moments"
	print(tab_wavemoments,include.rownames=T, hline.after= c(nrow(tab_wavemoments)), 
		  file=paste0(outputdir,"/",nametab,"_",wclab,".tex"))
	
	ageGrpDur_moments <- array(0.,dim=c(3,5))
	for(ai in seq(1,3)){
		ageGrpDur_moments[ai,1]   <- DTseam[ ageGrp==ai & EUfrq==T & ch==T , wtd.mean(max.unempdur_wave,weights=eval(as.name(wt)),na.rm=T)]
		ageGrpDur_moments[ai,2:5] <- DTseam[ ageGrp==ai & EUfrq==T & ch==T , wtd.4qtlmoments(max.unempdur_wave,eval(as.name(wt)))]
	}
	ageGrpDur_moments <- data.table(ageGrpDur_moments)
	names(ageGrpDur_moments) <- c("Mean","Median","Median Abs Deviaion","Goeneveld Meeden","Moors")
	rownames(ageGrpDur_moments) <- c("30 and under","31-55","56+")
	tab_wavemoments <- xtable(ageGrpDur_moments, digits=2, 
							  align="l|l|llll", caption="Distribution of quarter-quarter log earnings changes, absent unemployment")
	nametab <- "ageGrpDur_moments"
	print(tab_wavemoments,include.rownames=T, hline.after= c(nrow(tab_wavemoments)), 
		  file=paste0(outputdir,"/",nametab,"_",wclab,".tex"))
