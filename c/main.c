...
int const NE = 6;     // 8 number of firm epsilon match-quality types
...
double vftol = 1e-4;
double const rhotightening = 1.;
double const caltol = 1e-3;
double const smth_flows = 0.25;
double const smth_sw    = 0.005;
double const smth_wchng = 0.01;
...
int const TTT     = TT+burnin ;
int const Npaths  = 64;//160      // number of simulation paths to draw
int const Nsim    = 1000; // 1000

int const Npwave  = 4; // Periods per wave. MAKE SURE: TT/Npwave is an integer and TTT/Npwave is an integer

int const Nqtls   = 5; // number of quantiles that will use to compare distributions
int const Nextqtls = 9; //number of quantiles in the exprec U-shape, combines cental and edge quantiles
...
double       wage_lev = 1.0;    // will be a shifter so the average wage is >0
...
struct cal_params{
	...
	double lambdaEM_Rec, lambdaES_Rec, lambdaUS_Rec, lambdaUM_Rec; //probability of ability to make a move
    ...
    double delta_Exp;
	double zloss_Exp;   // expansion rate of zloss prob
    double delta_Rec   ;     // recession separation rate
	double lambdaUS_Exp,lambdaUM_Exp,lambdaES_Exp,lambdaEM_Exp; //How the move chance changes over the cycle
    double zloss_Rec;
...
struct hists{
    ...
    gsl_matrix     ** wchnghist_reset;
    gsl_matrix     ** swprob_hist;
    gsl_matrix     ** lambdaEM_hist;
    gsl_matrix     ** lambdaES_hist;
    gsl_matrix     ** lambdaUM_hist;
    gsl_matrix     ** lambdaUS_hist;
...
void memcpy_par(struct  cal_params *par_dest, struct  cal_params *par_orig);
...
void occ_shock_scenarios(struct cal_params * par, struct valfuns *vf, struct polfuns *pf, struct hists *ht, struct shocks *sk,struct stats * st ) ;
...
int main(int argc,char *argv[] ) {
    ...
    par.lambdaUS_Rec  = 0.2 / 0.5;
    par.lambdaUM_Rec  = 0.2 / 0.5;
    par.lambdaES_Rec = 0.01;
	par.lambdaEM_Rec = 0.8;
	par.kappa     = 0.00 ;
	par.gdfthr    = 0.5 ;
	par.wage_curve= 0.0 ;
	par.delta_Rec    = 0.01;
	par.delta_Exp = 0.;
	par.lambdaUS_Exp = 0.0;
    par.lambdaUM_Exp = 0.0;
    par.lambdaES_Exp = 0.0;
	par.lambdaEM_Exp = 0.0;
	par.eps_Acoef = -0.25;
    par.eps_Amag = 0.0;
    par.z_Acoef = -0.25;
    par.z_Amag = 0.0;
	...
	//, alphaE_1, alphaU_1
	par.param_lb[ii] = 0.010; par.param_ub[ii] = 0.95;ii++; //potentially these are not <1
	...
	if(Ncluster>1){
		...
		if(eps_2emg == 1){
			par.param_lb[ii]= 0.500;  par.param_ub[ii]= 5.00; ii++;
    ...
    par.cluster_hr= Ncluster;
    //baseline, print where we are +++++ was on the server
    //double dist = param_dist(par.xparopt, & par ,Nparams,err,Ntargets);

    // +++++
    // Scenarios stuff:
    int pl_0 = print_lev;    int vbs_0 = verbose; int opp_0 = opt_print;
    print_lev=0;    verbose=0; opt_print=0;
    success = draw_shocks(&sk);
    if(verbose>2 && success != 0) printf("Problem drawing shocks\n");
    success = sol_dyn(&par, &vf, &pf, &sk);
    if (verbose > 2 && success != 0) printf("Problem solving model\n");
    ...
    occ_shock_scenarios(&par,&vf, &pf, &ht, &sk, &st ) ;
    opt_print = opp_0;
    // +++++
    // END NEW
...
int sol_dyn( struct cal_params * par, struct valfuns * vf, struct polfuns * pf, struct shocks * sk ){
	...
	for(ii=0;ii<NUN;ii++){
		if( par->wage_curve< 0.0001 && par->wage_curve> -0.0001 )
			gg_set(vf0.WU,ii,0,
			       (1.-par->lambdaUS_Exp)*b
			       /(1.-beta) +
			       par->lambdaUS_Exp*(wagevec[ii*NE+NE/2][0])
			       /(1.-beta));
		else
			gg_set(vf0.WU,ii,0,
		               (1.-par->lambdaUS_Exp)*pow(b, 1.-par->wage_curve) / (1. - par->wage_curve)
		               /(1.-beta) +
		               par->lambdaUS_Exp*pow(wagevec[ii*NE+NE/2][0], 1.-par->wage_curve)/(1.-par->wage_curve)
		               /(1.-beta));
	...
	for(viter = 0;viter<maxiter;viter++){
		...
		for(ji=0;ji<JJ;ji++){
		    ...
		    for(oldE=0;oldE<2;oldE++) {
                ...
                for (ii = 0; ii < NN; ii++) {
                    ...
                    double lambdaEMhr = ai == 0 ? par->lambdaEM_Rec : par->lambdaEM_Exp;
                    double lambdaEShr = ai == 0 ? par->lambdaES_Rec : par->lambdaES_Exp; //par->lambdaES_Rec * exp(par->lambdaES_Exp * (double) ai);
                    lambdaEShr *= alphascl; //to scale it with alpha

                    double delta_hr   = ai == 0 ? par->delta_Rec : par->delta_Exp ; //par->delta_Rec * exp(par->delta_Exp * (double) ai);
                    double zloss_hr   = ai == 0 ? par->zloss_Rec : par->zloss_Exp; //par->zloss_Rec * exp(par->zloss_Exp * (double) ai);
		...
		for(ji=0;ji<JJ;ji++){
			...
			for(ii=0;ii<NUN;ii++) {
				...
				double lambdaUShr = ai==0 ? par->lambdaUS_Rec: par->lambdaUS_Exp;
                double lambdaUMhr = ai==0 ? par->lambdaUM_Rec: par->lambdaUM_Exp;
                double zloss_hr   = ai==0 ? par->zloss_Rec   : par->zloss_Exp   ; //relevant because might get a really bad z draw
                lambdaUShr *= alphascl; //to scale it as if also going through alpha
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	double delA0     = par->delta_Exp;
	double delm0     = par->delta_Rec;
	par->delta_Exp = delm0; //0.;
	par->delta_Rec = delm0; //delm0*exp(delA0);

	double zlossA0   = par->zloss_Exp;
	double zlossm0   = par->zloss_Rec;
	par->zloss_Exp   = zlossA0; ///0.;
	par->zloss_Rec   = zlossA0; //zlossm0*exp(zlossA0);

	double lamEMA0      = par->lambdaEM_Exp;
	double lamEMm0      = par->lambdaEM_Rec;
	par->lambdaEM_Rec   = lamEMA0;//lamEMm0*exp(lamEMA0);
	par->lambdaEM_Exp   = lamEMA0;

	double lamESA0      = par->lambdaES_Exp;
	double lamESm0      = par->lambdaES_Rec;
	par->lambdaES_Exp   = lamESA0;
	par->lambdaES_Rec   = lamESA0;

	double lamUSA0      = par->lambdaUS_Exp;
	double lamUSm0      = par->lambdaUS_Rec;
	par->lambdaUS_Rec   = lamUSA0; //lamUSm0*exp(lamUSA0);
	par->lambdaUS_Exp   = lamUSA0; //0.;

    double lamUMA0      = par->lambdaUM_Exp;
    double lamUMm0      = par->lambdaUM_Rec;
    par->lambdaUM_Rec   = lamUMA0; //lamUMm0*exp(lamUMA0);
    par->lambdaUM_Exp   = lamUMA0; //0.;
	...
	par->delta_Exp =delA0;
	par->zloss_Exp = zlossA0;
	par->lambdaEM_Exp = lamEMA0;
	par->lambdaES_Exp =lamESA0;
	par->lambdaUS_Exp = lamUSA0;
    par->lambdaUM_Exp = lamUMA0;

	par->delta_Rec    = delm0;
	par->zloss_Rec    = zlossm0;
	par->lambdaEM_Rec = lamEMm0;
	par->lambdaES_Rec =lamESm0;
	par->lambdaUS_Rec = lamUSm0;
    par->lambdaUM_Rec = lamUMm0;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	for(idx=0;idx<nscenario;idx++){
		...
		if(idx_ion[idx][0]==1){
            ...
            fprintf( scenariokey," 0,");
		if(idx_ion[idx][1]==1){
			//then we're turning off zloss_Exp
            par->zloss_Exp = zlossA0; ///0.;
            par->zloss_Rec = zlossA0; //zlossm0*exp(zlossA0);
            fprintf( scenariokey," 1,");
		} else
        ...
        if(idx_ion[idx][2]==1){
            ...
            fprintf(scenariokey, " 0,");
        if(idx_ion[idx][3]==1){
            //then we're turning off lambdaUM_Exp
            par->lambdaUM_Rec      = lamUMA0; //lamUMm0*exp(lamUMA0);
            par->lambdaUM_Exp = lamUMA0; //0.;
            fprintf( scenariokey," 1,");
        } else
            fprintf(scenariokey, " 0,");
        if(idx_ion[idx][4]==1){
            //then we're turning off lambdaES_Exp
            par->lambdaES_Exp = lamESA0;
            par->lambdaES_Rec      = lamESA0;
            fprintf( scenariokey," 1,");
        } else
            fprintf(scenariokey, " 0,");
        if(idx_ion[idx][5]==1){
			//then we're turning off lambdaEM_Exp
            par->lambdaEM_Rec = lamEMA0; //lamEMm0*exp(lamEMA0);
            par->lambdaEM_Exp = lamEMA0;//0.;
            fprintf( scenariokey," 1,");
		} else
            fprintf(scenariokey, " 0,");
    ...
    gsl_matrix * print_fexprec    = gsl_matrix_calloc(nscenario+1,Nextqtls);
    //do the decomposition, find the buddy of each:
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    for( idx=0;idx<nscenario;idx++ ){
        ...
        for(qi=0;qi<Nextqtls;qi++){
            gg_set(print_fexprec,idx,qi,st_nofX[idx].exprec_qtls[qi]);
        }
    ...
    for(qi=0;qi<Nextqtls;qi++)
        gg_set(print_fexprec, nscenario, qi,st->exprec_qtls[qi]);

    printmat("flow_disp_rec.csv",print_fdispstats,0);
    printmat("flow_exprec_qtls.csv",print_fexprec,0);
...
int sim( struct cal_params * par, struct valfuns *vf, struct polfuns *pf, struct hists *ht, struct shocks *sk ){
	...
	for(distiter = 0;distiter<4;distiter++){
		...
		for(ll=0;ll<Npaths;ll++){
            ...
            // set At only once per wave
            int wi;
            for(wi=0;wi<(TTT/Npwave);wi++) {
                ti = wi*Npwave;
                // increment shocks
                Atm1 = At;
                At = 0;
                for (ai = 0; ai < NA; ai++) if (gsl_vector_get(sk->Asel[ll], wi*Npwave) > cmAtrans[Atm1][ai]) ++At;
                for (ji = 0; ji < JJ; ji++) {
                    Ptm1[ji] = Pt[ji];
                    Pt[ji] = 0;
                    for (ai = 0; ai < NP; ai++) if (gg_get(sk->Psel[ll], wi*Npwave, ji) > cmPtrans[ji][Ptm1[ji]][ai]) ++Pt[ji];
                    for(si=0;si<Npwave;si++)
                        Pjhist[ji][si+wi*Npwave] = Pt[ji];
                }
                for(si=0;si<Npwave;si++) {
                    lambdaEMhist[si + wi * Npwave] = At == 0 ? par->lambdaEM_Rec          : par->lambdaEM_Exp ;
                    lambdaEShist[si + wi * Npwave] = At == 0 ? par->lambdaES_Rec*alphascl : par->lambdaES_Exp*alphascl;
                    lambdaUShist[si + wi * Npwave] = At == 0 ? par->lambdaUS_Rec*alphascl : par->lambdaUS_Exp*alphascl;
                    lambdaUMhist[si + wi * Npwave] = At == 0 ? par->lambdaUM_Rec          : par->lambdaUM_Exp;
                    deltahist[si + wi * Npwave] = At == 0 ? par->delta_Rec : par->delta_Exp;
                    zlosshist[si + wi * Npwave] = At == 0 ? par->zloss_Rec : par->zloss_Exp;
                }
                for(si=0;si<Npwave;si++) {
                    Athist[si + wi * Npwave] = At;
                    if (si + wi * Npwave >= burnin) gsl_vector_int_set(ht->Ahist[ll], si + wi * Npwave - burnin, At);
                }
                // moving forward latent At, even if that's never realized
                for(si=0;si<Npwave;si++) {
                    Atm1 = At;
                    At = 0;
                    for (ai = 0; ai < NA; ai++) if (gsl_vector_get(sk->Asel[ll], si+wi*Npwave) > cmAtrans[Atm1][ai]) ++At;
                }
            ...
            for(i=0;i<Nsim;i++){
                ...
                for(ti=0;ti<TTT;ti++) {
                    ...
                    if(ti - burnin>0) {
                        gg_set(ht->lambdaEM_hist[ll], i, ti - burnin, lambdaEM_hr);
                        gg_set(ht->lambdaUM_hist[ll], i, ti - burnin, lambdaUM_hr);
                        gg_set(ht->lambdaES_hist[ll], i, ti - burnin, lambdaES_hr);
                        gg_set(ht->lambdaUS_hist[ll], i, ti - burnin, lambdaUS_hr);
                    }
                    ...
                    if (utm1[i] == 0) {
                        ...
                        if (delta_hr > gg_get(sk->dsel[ll], i, ti)
                            ...
                            ut[i] = 0;
                            // stay or go?
                            if (ti >= burnin) gg_set(ht->swprob_hist[ll], i, ti - burnin, gg_get(pf->mE, ii, jt[i]));
                            //RE setting up:
                            double cumsij[JJ];
                            double alphasij[JJ];
                            double sEhr[JJ];
                                ...
                                if (jt[i] != jtm1[i]) { // switchers
                                    ...
                                    if (gg_get(sk->lambdaEMsel[ll], i, ti) < lambdaEM_hr) {
                                        ...
                                        if (gg_get(vf->WE, ip, jt[i]) >= gg_get(vf->WE, ipp, jt[i]) ||
                                            ...
                                            xSt[i] = xti1;
                                            zt[i] = zipp;
                                            //zt[i] = zipp==NZ-2 ? NZ-1 : zipp; //ad hoc adjustment! From <- to ,|
                                            epst[i] = epspp;
                    ...
                    if (utm1[i] == 1) {  // utm1 ==1 unemployed. Can search only in second period of job loss
                        ...
                        // stay or go?
                        if (ti >= burnin) gg_set(ht->swprob_hist[ll], i, ti - burnin, gg_get(pf->mU, ii, jt[i]));
                        //RU setting up
    ...
    if(print_lev >=2){
		...
		for(ll=0;ll<Npaths;ll++){
			...
			printmat(matname, ht->swprob_hist[ll],append );
	...
	for(ll=0;ll<Npaths;ll++){
		gsl_matrix_free(epssel_hist[ll]);
...
int sum_stats_flows(struct cal_params * par, struct valfuns *vf, struct polfuns *pf, struct hists *ht, struct shocks *sk, struct stats *st  ){
    ...
    for (ri = 0; ri < 3; ri++) { //run this 3 times for (0) expansion, (1) recession and (2) pooled
        ...
        reduction(+:NErisksep) reduction(+:Nocc_ten)
        for (ll = 0; ll < Npaths; ll++) {
...
int sum_stats(struct cal_params *par, struct valfuns *vf, struct polfuns *pf, struct hists *ht, struct shocks *sk,
    ...
    int Nsep_rec[2] = {0, 0}, NErisksep_rec[2] = {0, 0};
    double Nocc_ten = 0.;
    double PrswU = 0., PrswE = 0., PrnswE=0.;  double PrswU_rec[2] = {0.,0.}, PrswE_rec[2] = {0.,0.}, PrnswE_rec[2] = {0.,0.};
    ...
    //double avgalpha = 0.; double avgalphaN=0.;
    ...
    for (ri = 0; ri < 3; ri++) { //run this 3 times for (0) expansion, (1) recession and (2) pooled
        ...
        for (ll = 0; ll < Npaths; ll++) {
            ...
            int count_wi;
            double PrswU_ll = 0., PrswE_ll=0.,PrnswE_ll=0.;
            ...
            for (i = 0; i < Nsim; i++) {
                ...
                for (wi = 0; wi < (TT / Npwave); wi++) {//first loop over waves (wi), then loop over reference month (si)
                    ...
                    int Nemp_wi = 0, Nspell_wi = 0, Nfnd_wi = 0, NJ2J_wi = 0, NswE_wi = 0, NswSt_wi = 0, Nnosep_wi = 0, Nunemp_wi = 0, NswU_wi = 0, Ndur_sw_wi = 0, Ndur_nosw_wi = 0;
                    int Nsep_wi = 0, NErisksep_wi = 0;
                    double PrswU_wi=0.,PrswE_wi =0.,PrnswE_wi=0.;
                    for (si = 0; si < Npwave; si++) {
                        ti = si + wi * Npwave;
                        // should I count this observation? for ri=2, always true. For ri=0 only true when expansion and for ri=1 only true when recession
                        if (ri >= recIndic[gsl_vector_int_get(ht->Ahist[ll], ti)] && count_wi == 0)
                        ...
                        if (ti < TT - 1 && ti > 0) {
                            if (ggi_get(ht->uhist[ll], i, ti) == 0) {
                                ...
                                if (ggi_get(ht->uhist[ll], i, ti + 1) == 1) {
                                    ...
                                    Nnosep_wi = 1;
                                    double alphasiji_sum =0.;
                                    for (jji = 0; jji < JJ; jji++) alphasiji_sum += gg_get(ht->alphasijhist[ll][jji], i, ti);
                                    //avgalpha +=alphasiji_sum; avgalphaN +=1.;

                                    PrswE_wi += gg_get(ht->swprob_hist[ll],i,ti)*gg_get(ht->lambdaEM_hist[ll],i,ti)*alphasiji_sum;
                                    PrnswE_wi += (1.-gg_get(ht->swprob_hist[ll],i,ti))*gg_get(ht->lambdaES_hist[ll],i,ti);
                                    if (ggi_get(ht->J2Jhist[ll], i, ti) == 1) {
                                        NJ2J_wi = 1;
                                        if (ri == 2) NJ2Jmo++;
                                        if (ggi_get(ht->jhist[ll], i, ti - 1) != ggi_get(ht->jhist[ll], i, ti)) {
                                ...
                                double alphasiji_sum =0.;
                                for (jji = 0; jji < JJ; jji++) {
                                    alphasiji_sum += gg_get(ht->alphasijhist[ll][jji], i, ti);
                                }
                                PrswU_wi += gg_get(ht->swprob_hist[ll],i,ti)*gg_get(ht->lambdaUM_hist[ll],i,ti)*alphasiji_sum;
    ...
    //mix in the probabilistic part:
    st->swProb_U  = Nspell > 0 ? st->swProb_U *(1.-smth_sw) + smth_sw*  PrswU / (double) Nspell : 0.;
    //                                                                          This is ad hoc: I can't really get the prob that reject a J2J b/c value too low
    st->swProb_EE = NJ2J   > 0 ? st->swProb_EE*(1.-smth_sw) + smth_sw* ( PrswE / (PrswE+PrnswE)/2. + PrswE/2./(double)NJ2J*(par->gdfthr*2.) ): 0.;
    ...
    double swProb_EE_rec0 = NJ2J_rec[0] > 0 ? (double) NswE_rec[0] / (double) NJ2J_rec[0] * (1.-smth_sw) + smth_sw*PrswE_rec[0]/((double) NJ2J_rec[0])*par->gdfthr*2. : 0.;
    double swProb_EE_rec1 = NJ2J_rec[1] > 0 ? (double) NswE_rec[1] / (double) NJ2J_rec[1] * (1.-smth_sw) + smth_sw*PrswE_rec[1]/((double) NJ2J_rec[1])*par->gdfthr*2.: 0.;

    st->swProb_EE_ratio = swProb_EE_rec1 > 0 ? swProb_EE_rec0 / swProb_EE_rec1 : 0.;


    double swProb_U_rec0 = Nspell_rec[0] > 0 ? (double) NswU_rec[0] / (double) Nspell_rec[0]* (1.-smth_sw) + smth_sw*PrswU_rec[0]/((double) Nspell_rec[0]) : 0.;
    double swProb_U_rec1 = Nspell_rec[1] > 0 ? (double) NswU_rec[1] / (double) Nspell_rec[1]* (1.-smth_sw) + smth_sw*PrswU_rec[1]/((double) Nspell_rec[1]) : 0.;
    st->swProb_U_ratio = swProb_U_rec1 > 0 ? swProb_U_rec0 / swProb_U_rec1 : 0;
...
// now compute flows for EU transitions
    ...
    idx_all_rec[1] = 0;
    int idx_wall =0;
    int idx_wall_rec[2]; idx_wall_rec[0]=0;idx_wall_rec[1]=0;
...
//	#   omp parallel for private(ll,ti,i,wi,si,ji,jji) need to make idx' w/in each branch
        ...
        for (i = 0; i < Nsim; i++) {
            for (wi = 0; wi < TT / Npwave; wi++) {
                ...
                if (wi > 3 && wi < TT / Npwave - 3 && wnext > 0. && wlast > 0.) {
                    ...
                    int rri;double wlast_reset=0.,wnext_reset=0.;
                    ti = 0 + wi * Npwave;
                    for(rri=1; rri < Npwave * 3 + 1; rri++)
                        wlast_reset += exp(par->epslev->data[ ggi_get(ht->epshist[ll],i,ti-rri)]+par->zlev->data[ggi_get(ht->zhist[ll],i,ti-rri)]
                                           +par->xSlev->data[ggi_get(ht->xShist[ll],i,ti-rri)]+occ_wlevs[ ggi_get(ht->jhist[ll],i,ti-rri)] + par->Alev->data[gsl_vector_int_get(ht->Ahist[ll],ti-rri)]*par->AloadP->data[ggi_get(ht->jhist[ll],i,ti-rri)]
                                           +par->Plev->data[ggi_get(ht->Phist[ll],i,ti-rri) ]);
                    for(rri=0; rri < Npwave * 3 + 1; rri++)
                        wnext_reset += exp(par->epslev->data[ ggi_get(ht->epshist[ll],i,ti+rri)]+par->zlev->data[ggi_get(ht->zhist[ll],i,ti+rri)]
                                           +par->xSlev->data[ggi_get(ht->xShist[ll],i,ti+rri)]+occ_wlevs[ ggi_get(ht->jhist[ll],i,ti+rri)] + par->Alev->data[gsl_vector_int_get(ht->Ahist[ll],ti+rri)]*par->AloadP->data[ggi_get(ht->jhist[ll],i,ti+rri)]
                                           +par->Plev->data[ggi_get(ht->Phist[ll],i,ti+rri) ]);
                    for (si = 0; si < Npwave; si++) {
                        ti = si + wi * Npwave;
                        gg_set(ht->wchnghist_reset[ll], i, ti,
                               log(wnext_reset + pow(1. + wnext_reset * wnext_reset, 0.5)) -
                               log(wlast_reset + pow(1. + wlast_reset * wlast_reset, 0.5)));
                    }
                    if (isfinite(wchng)) {
                    ...
                    for (si = 0; si < Npwave; si++) {
                        ti = si + wi * Npwave;
                        gg_set(ht->wchnghist[ll], i, ti, -999);
                    }
                }
                ...
                if (isfinite(wchng) && wi > 3 && wi < TT / Npwave - 3 && wnext > 0. && wlast > 0.) {
                    if( I_stsw_wi == 1 || I_UEsw_wi == 1 || I_EUsw_wi == 1 || I_EEsw_wi == 1 ||
                        I_stns_wi == 1 || I_UEns_wi == 1 || I_EUns_wi == 1 || I_EEns_wi == 1){
                        w_all[idx_wall] = wchng;
                        w_all_rec[ri_wv][idx_wall_rec[ri_wv]] = wchng*(1.-smth_wchng) + smth_wchng*gg_get(ht->wchnghist_reset[ll],i,ti);
                        idx_wall ++;
                        idx_wall_rec[ri_wv] ++;
                    }
    ...
    w_qtls(w_working, 1, idx_wall-1, qtlgrid, st->all_qtls);
    w_qtls(w_working, 1, idx_wall-1, edgeqtls, st->edge_qtls);
    ...
    st->var_wg = gsl_stats_variance(w_working, 1, idx_wall);
    ...
    st->exprec_qtls[0] = st->edge_qtls_rec[0][0] - st->edge_qtls_rec[1][0];
    st->exprec_qtls[1] = st->edge_qtls_rec[0][1] - st->edge_qtls_rec[1][1];
    for(i=0;i<Nqtls;i++) st->exprec_qtls[i+2] = st->all_qtls_rec[0][i] - st->all_qtls_rec[1][i];
    st->exprec_qtls[7] = st->edge_qtls_rec[0][3] - st->edge_qtls_rec[1][3];
    st->exprec_qtls[8] = st->edge_qtls_rec[0][4] - st->edge_qtls_rec[1][4];
...
// this is the flip of set_params that goes from x to params. This takes params and loads an x
    ...
    if(ci==2 || ci == Ncluster){
        x[ii] = par->delta_Exp    ;ii++;
        x[ii] = par->lambdaEM_Exp ;ii++;
        x[ii] = par->lambdaES_Exp ;ii++;
        x[ii] = par->lambdaUM_Exp ;ii++;
        x[ii] = par->lambdaUS_Exp ;ii++;
        x[ii] = par->zloss_Exp    ;ii++;
        x[ii] = par->z_Acoef        ;ii++;
        x[ii] = par->z_Amag         ;ii++;
...
void print_targets(char *name, struct stats * st, struct stats * dat){
    ...
    //cyclical output:
    // +++++++++++++++++++++++++++++++++++++++++
    printarray("exprec_qtls_baseline.csv",st->exprec_qtls,Nextqtls,0);
...
/*
    ...
    fprintf(tgtfile,"MVswskew_exp,%f,%f \n", stMVswrec0_skew,  datMVswrec0_skew);
    fprintf(tgtfile,"MVsw50025_rec,%f,%f \n", stMVswrec1_bigloss, datMVswrec1_bigloss);
    fprintf(tgtfile,"MVsw50025_exp,%f,%f \n", stMVswrec0_bigloss, datMVswrec0_bigloss);
    fprintf(tgtfile,"MVns9505_rec,%f,%f \n", stMVnsrec1_p95p05, datMVnsrec1_p95p05);
    fprintf(tgtfile,"MVns9505_exp,%f,%f \n", stMVnsrec0_p95p05, datMVnsrec0_p95p05);
    fprintf(tgtfile,"MVnsskew_rec,%f,%f \n", stMVnsrec1_skew, datMVnsrec1_skew);
...
void occ_shock_scenarios(struct cal_params * par, struct valfuns *vf, struct polfuns *pf, struct hists *ht, struct shocks *sk,struct stats * st ) {
    ...
    // simulate this with various configurations to understand occupations' role

    // first try without any z-loss shocks, only employer loss-shocks
    // then try without any z-upgrades, only employer upgrades
    // then try with z-grid as epsilon grid

    int ll, i, it, wi,ri;

    int nscenario = 3;
    struct shocks *sk_scenX = malloc(sizeof(struct shocks) * nscenario);
    struct hists *ht_scenX = malloc(sizeof(struct hists) * nscenario);
    struct valfuns *vf_scenX = malloc(sizeof(struct valfuns) * nscenario);
    struct polfuns *pf_scenX = malloc(sizeof(struct polfuns) * nscenario);
    struct stats *st_scenX = malloc(sizeof(struct stats) * nscenario);

    struct cal_params * par_scenX = malloc(sizeof(struct cal_params) * nscenario);

    printf("In scenario tests! \n +++++++++++++++++++++\n +++++++++++++++++");

    for (i = 0; i < nscenario; i++)
        alloc_qtls(&st_scenX[i]);
    ...
    for (idx = 0; idx < nscenario; idx++) {
        ...
        allocate_mats(&vf_scenX[idx], &pf_scenX[idx], &ht_scenX[idx], &sk_scenX[idx]);
        allocate_pars(&par_scenX[idx]);

        memcpy_pf(&pf_scenX[idx], pf);
        memcpy_vf(&vf_scenX[idx], vf);
        memcpy_shocks(&sk_scenX[idx], sk);

        memcpy_par(&par_scenX[idx],par);

        if(idx ==0){
            // make z-shocks go away, and only employer-loss shocks:
            par_scenX[idx].delta_Exp += par -> zloss_Exp;
            par_scenX[idx].delta_Rec += par -> zloss_Rec;

            par_scenX[idx].zloss_Exp = 0.;
            par_scenX[idx].zloss_Rec = 0.;
        }
        if(idx ==1){
            // make z-EE go away:
            par_scenX[idx].lambdaEM_Exp += par->lambdaES_Exp;
            par_scenX[idx].lambdaEM_Rec += par->lambdaES_Rec;

            par_scenX[idx].lambdaES_Exp = 0.;
            par_scenX[idx].lambdaES_Rec = 0.;
        }
        if(idx ==2){
            // make z take the epsilon grid
            gsl_vector_memcpy(par_scenX[idx].zlev,par->epslev);
            for(ri=0;ri<NA;ri++)
                gsl_vector_memcpy(par_scenX[idx].zprob[ri],par_scenX[idx].epsprob[ri]);
        }
        print_lev=0;verbose=1; opt_print =0;
        //sol_dyn( par_scenX[idx], vf_scenX[idx
        sim( &par_scenX[idx], &vf_scenX[idx], &pf_scenX[idx], &ht_scenX[idx], &sk_scenX[idx] );
        sum_stats(& par_scenX[idx], &vf_scenX[idx],&pf_scenX[idx],&ht_scenX[idx],&sk_scenX[idx], &st_scenX[idx]);


        char arrayname[30];
        sprintf(arrayname, "stns_qtls_scen%d.csv",idx);
        printarray(arrayname, st_scenX[idx].stns_qtls, Nqtls, 0);
        sprintf(arrayname, "stsw_qtls_scen%d.csv",idx);
        printarray(arrayname, st_scenX[idx].stsw_qtls, Nqtls, 0);
        sprintf(arrayname, "EEns_qtls_scen%d.csv",idx);
        printarray(arrayname, st_scenX[idx].EEns_qtls, Nqtls, 0);
        sprintf(arrayname, "EEsw_qtls_scen%d.csv",idx);
        printarray(arrayname, st_scenX[idx].EEsw_qtls, Nqtls, 0);
        sprintf(arrayname, "EUEns_qtls_scen%d.csv",idx);
        printarray(arrayname, st_scenX[idx].EUEns_qtls, Nqtls, 0);
        sprintf(arrayname, "EUEsw_qtls_scen%d.csv",idx);
        printarray(arrayname, st_scenX[idx].EUEsw_qtls, Nqtls, 0);
        sprintf(arrayname, "EUns_qtls_scen%d.csv",idx);
        printarray(arrayname, st_scenX[idx].EUns_qtls, Nqtls, 0);
        sprintf(arrayname, "EUsw_qtls_scen%d.csv",idx);
        printarray(arrayname, st_scenX[idx].EUsw_qtls, Nqtls, 0);
        sprintf(arrayname, "UEns_qtls_scen%d.csv",idx);
        printarray(arrayname, st_scenX[idx].UEns_qtls, Nqtls, 0);
        sprintf(arrayname, "UEsw_qtls_scen%d.csv",idx);
        printarray(arrayname, st_scenX[idx].UEsw_qtls, Nqtls, 0);


        //put them all in one file if we're at the optimal
        sprintf(arrayname, "exprec_qtls_scen%d.csv",idx);

        printarray(arrayname,st_scenX[idx].exprec_qtls,Nextqtls,0);



        free_mats(&vf_scenX[idx],&pf_scenX[idx],&ht_scenX[idx],&sk_scenX[idx]); // not releasing st
        print_lev = pl_0; verbose =vbs_0;
    }

    for(i=0;i<nscenario;i++){
        free_qtls(&st_scenX[i]);
    }
...
void compute_derivs(double**cal_Jacobian, struct cal_params * par0, int printJac ){
    ...
    for(xi=0;xi<Nparams;xi++){
        ...
        xp_pspace[xi] = gsl_min( x_pspace[xi] +
                0.05*( par0->param_ub[xi]- par0->param_lb[xi]),
                par0->param_ub[xi]);
...
void alloc_hists( struct hists *ht ){
    ...
    ht->wchnghist_reset = malloc(sizeof(gsl_matrix      *)*Npaths);
    ...
    ht->swprob_hist = malloc(sizeof(gsl_matrix      *)*Npaths);
    ht->lambdaEM_hist = malloc(sizeof(gsl_matrix      *)*Npaths);
    ht->lambdaES_hist = malloc(sizeof(gsl_matrix      *)*Npaths);
    ht->lambdaUM_hist = malloc(sizeof(gsl_matrix      *)*Npaths);
    ht->lambdaUS_hist = malloc(sizeof(gsl_matrix      *)*Npaths);
    ...
    for(ll=0;ll<Npaths;ll++){
        ...
        ht->wchnghist_reset[ll] = gsl_matrix_calloc(Nsim,TT);
        ...
        ht->swprob_hist[ll] = gsl_matrix_calloc(Nsim,TT);
        ht->lambdaEM_hist[ll] = gsl_matrix_calloc(Nsim,TT);
        ht->lambdaES_hist[ll] = gsl_matrix_calloc(Nsim,TT);
        ht->lambdaUM_hist[ll] = gsl_matrix_calloc(Nsim,TT);
        ht->lambdaUS_hist[ll] = gsl_matrix_calloc(Nsim,TT);
...
void alloc_qtls( struct stats *st ){
    ...
    st->exprec_qtls = malloc(Nextqtls*sizeof(double));
	nflows = fact_int(JJ)/(fact_int(2)*fact_int(JJ-2));
...
void free_hists( struct hists *ht ){
	...
	for(ll=0;ll<Npaths;ll++){
        ...
        gsl_matrix_free(ht->wchnghist_reset[ll]);
		...
		gsl_matrix_free(ht->swprob_hist[ll]);
        gsl_matrix_free(ht->lambdaEM_hist[ll]);
        gsl_matrix_free(ht->lambdaES_hist[ll]);
        gsl_matrix_free(ht->lambdaUM_hist[ll]);
        gsl_matrix_free(ht->lambdaUS_hist[ll]);
	}
	...
	free(ht->J2Jhist);
    free(ht->swprob_hist);
    free(ht->lambdaEM_hist);
    free(ht->lambdaES_hist);
    free(ht->lambdaUM_hist);
    free(ht->lambdaUS_hist);
...
void memcpy_par(struct  cal_params *par_dest, struct  cal_params *par_orig){
    ...
    int i,ii,ri,ji;
    memcpy(par_dest, par_orig, sizeof *par_dest);
    for (i=0;i<Nparams;i++){
        par_dest->param_lb[i] =par_orig->param_lb[i];
        par_dest->param_ub[i] =par_orig->param_ub[i];
        par_dest->xparopt[i] =par_orig->xparopt[i];
        par_dest->xsolopt[i] =par_orig->xsolopt[i];
    }

    for(i=0;i<JJ;i++){
        for(ii=0;ii<JJ;ii++) {
            par_dest->alpha_nf[i][ii] = par_orig->alpha_nf[i][ii];
//            for(ri=0;ri<2;ri++)
//              par_dest->alpha_nf_rec[ri][i][ii] = par_orig->alpha_nf_rec[ri][i][ii];
        }
    }

    gsl_vector_memcpy(par_dest->AloadP,par_orig->AloadP); //loading on A for each P
    gsl_vector_memcpy(par_dest->Plev,par_orig->Plev);
    gsl_vector_memcpy(par_dest->Alev,par_orig->Alev);
    gsl_vector_memcpy(par_dest->xSlev,par_orig->xSlev);
    gsl_vector_memcpy(par_dest->zlev,par_orig->zlev);
    gsl_vector_memcpy(par_dest->epslev,par_orig->epslev);
    for(ii=0;ii<JJ;ii++){
        gsl_matrix_memcpy(par_dest->Ptrans[ii],par_orig->Ptrans[ii]);
    }
    for(ii=0;ii<JJ;ii++){
        gsl_matrix_memcpy(par_dest->Ptrans[ii],par_orig->Ptrans[ii]);
    }
    ...
    gsl_matrix_memcpy(par_dest->Atrans,par_orig->Atrans);
    gsl_matrix_memcpy(par_dest->xStrans,par_orig->xStrans);

    for(ii=0;ii<NA;ii++)
        gsl_vector_memcpy(par_dest->epsprob[ii],par_orig->epsprob[ii]);
    gsl_vector_memcpy(par_dest->endogepsprob,par_orig->endogepsprob );

    for(ii=0;ii<NA;ii++)
        gsl_vector_memcpy(par_dest->UEepsprob[ii],par_orig->UEepsprob[ii]);
    for(ii=0;ii<NA;ii++)
        gsl_vector_memcpy(par_dest->zprob[ii] ,par_orig->zprob[ii]) ;
    gsl_vector_memcpy(par_dest->endogzprob,par_orig->endogzprob);

    gsl_vector_memcpy(par_dest->jprob,par_orig->jprob);

    for(ii=0;ii<Ntrx;ii++) {
        for (ji = 0; ji < Nqtls; ji++) par_dest->w_fac[ii][ji] = par_orig->w_fac[ii][ji];
    }
    for(ii=0;ii<4;ii++) {
        for (ji = 0; ji < Nqtls; ji++) par_dest->w_cycfac[ii][ji] = par_orig->w_cycfac[ii][ji] ;
    }
